...
func TestSpaceRequest(t *testing.T) {
	...
	_, err := alloc1.Allocate(container1, subnet, returnFalse)
	require.Nil(t, err, "")
	require.Equal(t, cidr(universe).Range(), alloc1.ring.OwnedRanges()[0], "")
	require.Equal(t, 1, len(alloc1.ring.OwnedRanges()))
	...
	require.Equal(t, cidr("10.32.0.0/13").Range(), alloc1.ring.OwnedRanges()[0], "")
	require.Equal(t, cidr("10.40.0.0/13").Range(), alloc2.ring.OwnedRanges()[0], "")
	require.Equal(t, 1, len(alloc1.ring.OwnedRanges()))
	require.Equal(t, 1, len(alloc2.ring.OwnedRanges()))
}

func cidr(s string) address.CIDR {
	_, c, _ := address.ParseCIDR(s)
	return c
}
